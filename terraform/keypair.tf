...
resource "aws_key_pair" "generated_key" {
  key_name   = "chave-fastfood"                                  # Nome da chave que será utilizada para acessar a instância EC2
...
resource "aws_secretsmanager_secret" "fastfood" {
  name = "keypair-fastfood" # Nome do segredo que será armazenado

   # Evita recriação se o segredo já existir
  lifecycle {
...
resource "aws_secretsmanager_secret_version" "fastfood" {
  secret_id = aws_secretsmanager_secret.fastfood.id # ID do segredo que estamos atualizando
